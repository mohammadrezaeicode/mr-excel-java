...
public class ColorUtils {
    ...
    public static Optional<String> rgbToHex(String rgb) {
        rgb = removeSpace(rgb);
        String[] spResult = rgb.indexOf("rgba") >= 0 ? rgb.substring(5, rgb.length() - 1).split(",") : rgb.substring(4, rgb.length() - 1).split(",");
        ...
        return Optional.of((valueToHex(parse.get(0)) + valueToHex(parse.get(1)) + valueToHex(parse.get(2))).toUpperCase());
    }

    public static String convertToHex(String fgConvertor) {
        if (fgConvertor == null) {
            return "";
        }
